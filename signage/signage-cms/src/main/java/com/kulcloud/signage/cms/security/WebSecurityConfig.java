...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Value("${com.kulcloud.signage.publicApiPath}")
...
//			.anyRequest().hasAnyAuthority(userService.getAllRoleIds())
	...
	boolean isPermitAllRequest(HttpServletRequest request) {
		return StringUtils.isEmpty(apiPath) || 
				StringUtils.containsAny(request.getPathInfo(), apiPath) || 
				StringUtils.containsAny(request.getPathInfo(), "/cast", "/ws", "/webjars");
	} 
}
