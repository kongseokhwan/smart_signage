...
@Route(value = "live", layout = MainView.class)
@PageView(title="Live Contents", icon="FILE_MOVIE")
@PageTitle("Live Contents")
public class LivePage extends ContentLayout {

	private static final long serialVersionUID = 1L;
	
	@Autowired
	private LiveContentRepository contentRepo;
	@Autowired
	private SignageTenantRepository tenantRepo;
	
	private Grid<LiveContent> contentGrid = new Grid<>(LiveContent.class);
	private Binder<LiveContent> contentBinder;
	...
	private Grid<LiveContent> createContentGrid() {
		contentGrid = new Grid<>(LiveContent.class);
		contentGrid.setWidthFull();
		contentGrid.setHeight(CommonConstants.innerGridHeight);
		contentGrid.setColumns("name", "title", "url", "liveType");
		contentGrid.setSelectionMode(SelectionMode.SINGLE);
		SingleSelect<Grid<LiveContent>, LiveContent> contentSelect = contentGrid.asSingleSelect();
	...
	private FormLayout createFormLayout() {
		...
		contentBinder = new Binder<>(LiveContent.class);
		...
		TextField fileField = new TextField("Live Type");
		contentBinder.bind(fileField, "liveType");
	...
	private void setForm(LiveContent content) {
		if(content != null) {
